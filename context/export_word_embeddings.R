...
for (file in files){
  ...
  hansard_vocab = prune_vocabulary(hansard_vocab, term_count_min = 50)
  ...
  hansard_tcm = create_tcm(it, vectorizer, skip_grams_window = 10)
  
  glove = GlobalVectors$new(rank = 4, x_max = 10)
  
  hansard_wv_main = glove$fit_transform(hansard_tcm, n_iter = 1000, convergence_tol = 0.00000001, n_threads = 12)
  ...
  kw = hansard_word_vectors["woman", , drop = F]
