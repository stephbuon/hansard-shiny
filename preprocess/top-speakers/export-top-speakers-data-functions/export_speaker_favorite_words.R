...
export_top_speakers_favorite_words <- function(tokenized_hansard, stopwords) {
  ...
  speaker_names_list <- top_speakers$new_speaker
  ...
  for(decade in 1:length(decades)) {
    for(speaker_name in 1:length(speaker_names_list)) {
      ...
      filtered_speaker_name$speaker <- speaker_name # to make names consistent
