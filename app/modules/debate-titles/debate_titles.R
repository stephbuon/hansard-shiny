...
debate_titles_server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    words_per_year <- fread(paste0(data_dir, "debates/words_per_year.csv"))
    debate_title_year_counts <- fread(paste0(data_dir, "debates/debate_title_year_counts.csv"))
    
    
    import_debate_titles_data <- reactive( {
      if (input$kw_list != "custom") {
        kw_data <- fread(paste0(data_dir, "debates/kw_list_", input$kw_list, ".csv")) } 
      else { 
        kw_data <- data.table() } })
    ...
    m <- fread(paste0(data_dir, "debates/all_debate_titles_metadata.csv")) %>%
    ...
    render_value_debate_titles = function(input_kw_list, debate_titles_data, ...){
      output$debate_titles_DT <- renderDT({
        ...
        if (input$kw_list != "custom") {
          metadata <- fread(paste0(data_dir, "debates/kw_metadata_", input_kw_list, ".csv")) 
