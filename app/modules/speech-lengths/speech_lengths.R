...
speech_lengths_server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    viz <- fread(paste0(data_dir, "speakers/speech_lengths_overview.csv"))
    hansard_speech_lengths <- fread(paste0(data_dir, "speakers/speech_lengths.csv")) #%>%
    ...
    render_text <- function(d){
      ...
      output$plot_2 <- renderPlotly({
        ...
        if (input$drop_down_hist == "overview") {
          ...
          a <- fread(paste0(data_dir, "debate-text/tokenized_hansard_count/tokenized_hansard_count_all_decades_speech_length_type_0.csv")) %>%
            filter(ngrams == word)


          b <- fread(paste0(data_dir, "debate-text/tokenized_hansard_count/tokenized_hansard_count_all_decades_speech_length_type_1.csv")) %>%
            filter(ngrams == word)


          c <- fread(paste0(data_dir, "debate-text/tokenized_hansard_count/tokenized_hansard_count_all_decades_speech_length_type_2.csv")) %>%
            filter(ngrams == word)
